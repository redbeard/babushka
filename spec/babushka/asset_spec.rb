...
describe Asset do
  ...
  it "should attempt to detect type via `file` when there is no extension" do
    ShellHelpers.should_receive(:shell).with("file '#{archive_path / 'zip_without_extension'}'").any_number_of_times.and_return('Zip archive data')
  ...
  it "should raise an error on unsupported types" do
    ShellHelpers.should_receive(:shell).with("file '#{archive_path / 'invalid_archive'}'").any_number_of_times.and_return('ASCII text')
    L{
